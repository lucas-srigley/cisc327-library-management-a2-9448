...
def test_pay_late_fees_exceeds_payment_limit(mocker):
    mocker.patch('services.library_service.calculate_late_fee_for_book', return_value={'fee_amount': 1001, 'days_overdue': 14, 'status': 'overdue'})
    mocker.patch('services.library_service.get_book_by_id', return_value={'id': 1, 'title': 'Test Book', 'author': 'Test Author'})
    mock_gateway = Mock(spec=PaymentGateway)
    mock_gateway.process_payment.return_value = (False, "txn_123456", "Payment declined: amount exceeds limit")
    success, message, transaction_id = pay_late_fees("123456", 1, mock_gateway)
    mock_gateway.process_payment.assert_called_once_with(
        patron_id="123456",
        amount=1001,
...
def test_pay_late_fees_invalid_type(mocker):
    mocker.patch('services.library_service.calculate_late_fee_for_book', return_value="invalid_data")
...
def test_pay_late_fees_returns_none(mocker):
    mocker.patch('services.library_service.calculate_late_fee_for_book', return_value={'fee_amount': 7.0, 'days_overdue': 3, 'status': 'overdue'})
    mocker.patch('services.library_service.get_book_by_id', return_value={'id': 1, 'title': 'Test Book'})
    mock_gateway = Mock(spec=PaymentGateway)
    mock_gateway.process_payment.return_value = None
    success, message, transaction_id = pay_late_fees("123456", 1, mock_gateway)
    assert not success
    assert "Payment processing error" in message
    assert transaction_id is None
...
def test_refund_late_fee_invalid_return_type(mocker):
    mock_gateway = Mock(spec=PaymentGateway)
    mock_gateway.refund_payment.return_value = "Refund completed"
