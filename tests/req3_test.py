...
def test_borrow_book_valid_input():
    ...
    success, message = borrow_book_by_patron("123456", get_book_by_isbn("1234567890124")["id"])
    assert success == True
...
def test_borrow_book_invalid_patron_id_empty():
    ...
    success, message = borrow_book_by_patron("", get_book_by_isbn("1234567890124")["id"])
...
def test_borrow_book_invalid_patron_id_too_long():
    ...
    success, message = borrow_book_by_patron("1234567", get_book_by_isbn("1234567890124")["id"])
    assert success == False
    assert "Invalid patron ID. Must be exactly 6 digits." in message

def test_borrow_book_invalid_patron_id_not_an_integer():
    """Test borrowing a book with patron id not an integer."""
    success, message = borrow_book_by_patron("a123456", get_book_by_isbn("1234567890124")["id"])
    assert success == False
    assert "Invalid patron ID. Must be exactly 6 digits." in message
...
def test_borrow_book_invalid_borrow_limit_exceeded():
    """Test borrowing a book with valid input."""
    success, message = borrow_book_by_patron("123456", get_book_by_isbn("1234567890123")["id"])
    assert success == True
    assert "successfully borrowed" in message.lower()
    success, message = borrow_book_by_patron("123456", get_book_by_isbn("1234567890123")["id"])
    assert success == True
